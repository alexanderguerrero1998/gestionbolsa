...
var baseFixedColumns = {
    ...
    synchronizeRows: function() {
        ...
        this.waitAsyncTemplates(true).done((function() {
            if (_this3._isFixedColumns && _this3._tableElement && _this3._fixedTableElement) {
                var heightTable = _this3._getClientHeight(_this3._tableElement.get(0));
                var heightFixedTable = _this3._getClientHeight(_this3._fixedTableElement.get(0));
                $rowElements = _this3._getRowElements(_this3._tableElement);
                $fixedRowElements = _this3._getRowElements(_this3._fixedTableElement);
                $contentElement = _this3._findContentElement();
                if (heightTable !== heightFixedTable) {
                    $contentElement && $contentElement.css("height", heightTable);
                    $rowElements.css("height", "");
                    $fixedRowElements.css("height", "");
                    for (rowIndex = 0; rowIndex < $rowElements.length; rowIndex++) {
                        rowHeights.push(_this3._getClientHeight($rowElements.get(rowIndex)));
                        fixedRowHeights.push(_this3._getClientHeight($fixedRowElements.get(rowIndex)))
                    }
                    for (rowIndex = 0; rowIndex < $rowElements.length; rowIndex++) {
            ...
            if (true === columnFixingOptions.enabled && column && column.allowFixing) {
                var onItemClick = function(params) {
                    switch (params.itemData.value) {
                        case "none":
                            _this4._columnsController.columnOption(column.index, "fixed", false);
                            break;
                        case "left":
                            _this4._columnsController.columnOption(column.index, {
                                fixed: true,
                                fixedPosition: "left"
                            });
                            break;
